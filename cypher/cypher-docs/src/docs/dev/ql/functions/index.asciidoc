...
* Temporal functions - duration [<<header-query-functions-temporal-duration, Summary>>|<<query-functions-temporal-duration, Detail>>]
...
1.2+| <<functions-isnan, `isNaN()`>> | `isNaN(input :: FLOAT?) :: (BOOLEAN?)` | Returns whether a floating point number is `NaN`.
| `isNaN(input :: INTEGER?) :: (BOOLEAN?)` | Returns whether an integer number is `NaN`.
...
1.1+|<<functions-withinBBox,`pointwithinBBox()`>> | `point.withinBBox(point :: POINT?, lowerLeft :: POINT?, upperRight :: POINT?) :: (BOOLEAN?)`  | Returns true if the provided point is within the bounding box defined by the two provided points.
...
include::query-functions-temporal-duration.adoc[leveloffset=+1]
